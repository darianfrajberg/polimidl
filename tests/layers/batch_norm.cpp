...
int test(unsigned int number_of_workers){
  ...
  using polimidl::layers::batch_norm;
  ...
  alignas(buffer_alignment::byte_alignment) const float beta[] = {1,2,3}; //1x1x3
  alignas(buffer_alignment::byte_alignment) const float mean[] = {4,5,6}; //1x1x3
  //    const float gamma[] = {1,2,4}; //1x1x3
  //    const float variance[] = {4,9,16}; //1x1x3
  //    const float epsilon = 0.00001; //1x1x3
  float fused_gamma_variance_epsilon[] = {0.5,0.66,1}; //1x1x3

  {
    constexpr float expected_output[] = {-1,1.34,5,-0.5,2,6,0,2.66,7,0.5,3.32,8};

    auto net = build_network<float>(h, w, number_of_workers,
        batch_norm<float, components<channels>>(beta, mean,
                                         fused_gamma_variance_epsilon)
    );
    if (net.input_rows() != h || net.input_columns() != w || net.input_components() != channels) {
      std::cerr << "Invalid input shape for batch normalization: " <<
        net.input_rows() << "x" << net.input_columns() << "x" << net.input_components() << std::endl;
      return -1;
    }
    if (net.input().size() != std::size(input)) {
      std::cerr << "Invalid input size for batch normalization: " <<
          net.input().size() << std::endl;
      return -1;
    }
    if (net.output_rows() != h || net.output_columns() != w || net.output_components() != channels) {
      std::cerr << "Invalid output shape for batch normalization: " <<
    ...
    if (output.size() != std::size(expected_output)) {
      std::cerr << "Invalid output size for batch normalization: " <<
    ...
    if (!std::equal(output.begin(), output.end(),
                   ...
                   is_close)){
      std::cerr << "Invalid output value for batch normalization" << std::endl;
      return -1;
    }
  }
  return 0;
