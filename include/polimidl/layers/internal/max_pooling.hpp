#ifndef POLIMIDL_LAYERS_INTERNAL_MAX_POOLING_HPP
#define POLIMIDL_LAYERS_INTERNAL_MAX_POOLING_HPP
...
template <typename type_t, typename components, typename kernel,
          typename stride>
class max_pooling : public layer<type_t, components> {
 public:
  max_pooling() : batch_size_(1) {}
  ...
  static std::size_t temporary_size(std::size_t input_rows, std::size_t input_columns,
  ...
  void operator()(input_t input, temporary_t temporary, output_t output,
    ...
    if (input_rows == kernel::rows && input_columns == kernel::columns) {
      matrix_temp_t val(&input[0]);
      matrix_out_t out(&output[0]);
      out = val.rowwise().maxCoeff();
      return;
    }
    const std::size_t output_rows = max_pooling::output_rows(input_rows);
    const std::size_t output_columns = max_pooling::output_columns(input_columns);
    ...
    for (std::size_t start_cell = 0; start_cell < cells; start_cell += batch_size_) {
      ...
      scheduler.schedule([=] (std::size_t worker) {
        ...
        for (std::size_t cell = start_cell; cell < end_cell; ++cell) {
          ...
          matrix_temp_t val(&worker_temporary[0]);
          auto i_output = &output[components::value * cell];
          matrix_out_t out(i_output);
          out = val.rowwise().maxCoeff();
        }
      });
    }
 ...
 private:
  ...
  using matrix_temp_t = Eigen::Map<
      Eigen::Matrix<type_t, components::value, kernel::columns * kernel::rows>,
      alignment_t::eigen_alignment>;
  using matrix_out_t = Eigen::Map<
      Eigen::Array<type_t, components::value, 1>>;
...
#endif // POLIMIDL_LAYERS_INTERNAL_MAX_POOLING_HPP
