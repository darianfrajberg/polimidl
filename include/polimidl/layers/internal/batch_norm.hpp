#ifndef POLIMIDL_LAYERS_INTERNAL_BATCH_NORM_HPP
#define POLIMIDL_LAYERS_INTERNAL_BATCH_NORM_HPP
...
class batch_norm : public layer<type_t, components, true> {
 public:
  batch_norm(const type_t* beta, const type_t* mean,
  ...
  void operator()(input_t input, temporary_t temporary, output_t output,
    ...
    for (std::size_t start_cell = 0; start_cell < cells;
      ...
      scheduler.schedule([=] (std::size_t worker) {
        ...
        // Given the fact that input and output have the same size, and the
        // layer is in place, they are guaranted to be perfectly overlapping.
...
#endif // POLIMIDL_LAYERS_INTERNAL_BATCH_NORM_HPP
